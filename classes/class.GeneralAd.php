...
//The GeneralAd class implements the Ad interface
class GeneralAd implements Ad
{
    ...
    {
        $adQuery = $this->database->query('SELECT id, userid, type, title, price, description, tags, time, email, postal, deleted FROM advertisements WHERE id = ? AND deleted = 0 LIMIT 1', $adID); //Get the ad from the database
	...
	public function postAd($userID, $formData) //Add the ad to the database
    {
        ...
        if(strlen(trim($formData['title'])) >= 10 && strlen(trim($formData['title'])) <= 50 && is_numeric($formData['price']) && intval($formData['price']) >= 1 && intval($formData['price']) <= 1000000000 && strlen($formData['description']) >= 1 && strlen($formData['description']) <= 500 && strlen($formData['address']) >= 6 && strlen($formData['address']) <= 7 && strlen($formData['images'])) //The POST variables are valid
        {
            ...
            if($this->database->query('INSERT INTO advertisements (userid, type, title, price, description, tags, time, email, postal) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', $userID, 0, trim($formData['title']), $formData['price'], $formData['description'], $formData['tags'], $time, $formData['email'], $formData['address'])) //Insert the new ad into the database
            {
	...
	public function editAd($userID, $adID, $formData) //Edit the specified ad if the user owns it
    {
        ...
        if(strlen(trim($formData['title'])) >= 10 && strlen(trim($formData['title'])) <= 50 && is_numeric($formData['price']) && intval($formData['price']) >= 1 && intval($formData['price']) <= 1000000000 && strlen($formData['description']) >= 1 && strlen($formData['description']) <= 500 && strlen($formData['address']) >= 6 && strlen($formData['address']) <= 7) //The POST variables are valid
        {
            if($this->database->query('UPDATE advertisements SET title = ?, price = ?, description = ?, tags = ?, email = ?, postal = ? WHERE userid = ? AND id = ?', trim($formData['title']), $formData['price'], $formData['description'], $formData['tags'], $formData['email'], $formData['address'], $userID, $adID)) //Update the specified ad
            {
    ...
    {
        include(__DIR__ . '/../pages/generalform.php'); //Include the ad posting form
    }
    ...
    public function showEditView($userID, $adID) //Show the ad editing form view
    {
        ...
        {
            include(__DIR__ . '/../pages/editgeneralform.php'); //Include the ad edit form
        }
    ...
    public function showAdView($adID, $posterName, $loggedIn) //Show the ad view
    {
        ...
        include(__DIR__ . '/../pages/generaladview.php'); //Show the ad view
    }
}